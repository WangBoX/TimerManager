...
- (void)clickSingle:(id)sender{
    ...
    BIWeakObj(self)
    [[WBGCDTimerManager sharedInstance] scheduleGCDTimerWithName:@"beck.wang.singleTimer" interval:1 queue:dispatch_get_main_queue() repeats:YES option:CancelPreviousTimerAction action:^{
...
- (void)clickCountDown{
    _curCountDown -= 1;
    if (_curCountDown < 0) {
        [[WBGCDTimerManager sharedInstance] cancelTimerWithName:@"beck.wang.singleTimer"];
        return ;
    }
    else{
