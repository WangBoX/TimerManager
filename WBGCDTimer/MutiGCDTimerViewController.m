...
- (void)dealloc{
    ...
    [self.addtimerArray enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
        [[WBGCDTimerManager sharedInstance] cancelTimerWithName:key];
    }];
}
...
- (void)addGCDTimerArry{
    ...
    for (int i = 0; i<10; i++) {
        ...
        [[WBGCDTimerManager sharedInstance] scheduleGCDTimerWithName:timerKey interval:1 queue:dispatch_get_main_queue() repeats:YES option:CancelPreviousTimerAction  action:^{
            ...
            if(totalCount < 0)
            {
                [[WBGCDTimerManager sharedInstance] cancelTimerWithName:timerKey];
